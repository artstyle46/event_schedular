...
def create_user():
    ...
    for i in range(10000):
        user_list.append(get_user_model().objects.create(username=f"asit_{i}", password="test"))
    return user_list
    ...
    def handle(self, *args, **options):
        ...
        for i in range(2000000):
            ...
            if field.field_type == "date":
                ...
                random_date = datetime(today.year, today.month, today.day, rand_hour, rand_minute)
                field_value = FieldValue(field=field, date_value=random_date, user=u)
                field_value.save()
            else:
                field_value = FieldValue(field=field, string_value=f"string_{i}", user=u)
                field_value.save()
        ...
        for i in range(date_len):
            ...
            trigger = Trigger(field=date_fields[i], time=f"{rand_hour}:{rand_minute}", period=period,
                              offset_type=offset_type, date_offset=date_offset)
            trigger.save()
